...
**Floating Sensor Hub** to system zbierania i przetwarzania danych z pojedynczego urządzenia IoT komunikującego się z centralnym hubem za pośrednictwem protokołu MQTT. W aktualnej wersji pomiary bazują na czujnikach BME688 i LSM6DSV (bez pomiaru impedancji). System przechowuje dane w bazie InfluxDB i udostępnia je poprzez panel operatorski NiceGUI oraz dashboardy Grafana.
...
|-----------|------------------|
| Mosquitto | Komunikacja MQTT |
| Python Backend | Logika systemu, API, automatyka |
| InfluxDB | Dane czasowe |
| SQLite | Konfiguracja i dane relacyjne |
| NiceGUI | Panel operatorski |
| Grafana | Wizualizacja historyczna i alerty |
...
Komunikacja pomiędzy kontenerami odbywa się wyłącznie po nazwach usług Docker, np.:
...
Do komunikacji z urządzeniami IoT wystawiane są jedynie niezbędne porty (MQTT, HTTPS/API).

### MQTT – zalecenia

- QoS 1 dla danych pomiarowych.
- `retain` dla bieżących stanów urządzeń.
- Last Will & Testament do wykrywania awarii urządzeń.
- Hierarchiczna struktura topiców:

```
sensor/<node>/measurement
sensor/<node>/diagnostic
sensor/<node>/event
